...
3. 组织cookie的数据结构       因为sku_id 是唯一值 所以需要唯一的key
   cart =  {
...
# 添加购物车的业务
    def post(self,request):
        ...
        if user is not None and user.is_authenticated:
            ...
            # redis_conn.hincrby('cart_%s'%user.id,sku_id,count)  # hincrby 累加保存在redis中的hash中
            # # set  cart_selected_userid: sku_id
            # if selected:
            #     redis_conn.sadd('cart_selected_%s'%user.id,sku_id)  # 保存在redis中的set中 因为都在redis中保存，所以需要优化
            ...
            # 管道② 将指令添加到管道中
            pl.hincrby('cart_%s' % user.id, sku_id, count)   # hincrby   hash方法个数累加
            # set  cart_selected_userid: sku_id
            if selected:
...
#  点击购物车列表 展示购物车内容
    def get(self,request):
        ...
        if user is not None and user.is_authenticated:
            ...
            # cart_userid ： {sku_id:count,sku_id:count,sku_id:count}
            # {1:5,2:5,3:5}
            #         set         cart_selected_userid:   sku_id
            #获取set数据
            ...
            for sku_id,count in redis_ids_count.items():
                cookie_cart[int(sku_id)]={         # redis里面是byts类型 ，所以得需要转换
                    'count':int(count),
            ...
            if cookie_str is not None:
                ...
                cookie_cart = {}
            #   cart_selecteduserid：{sku_id:{count:xxx,selected:xxxx},sku_id:{count:xxx,selected:xxxx},}
...
#  删除购物车
    ...
    def delete(self, request):
        ...
        if user is not None and user.is_authenticated:
            ...
            #     7.2 判断cart数据是否存在
            if cookie_str is not None:
                cookie_cart = pickle.loads(base64.b64decode(cookie_str))     # JSON类型
            else:
                cookie_cart = {}
            # 7.3 删除指定数据
            if sku_id in cookie_cart:
