...
#  JWT加密使用
SECRET_KEY = '@4juyj59)toa-vp+#l@@tvgm!4a+t^bc7aea!$!0tmjdeqvu)@'
...
INSTALLED_APPS = [
    ...
    # 定时任务
    'django_crontab',
    #drf-haystack封装接口调用Elasticsearch
    'haystack',
    # 订单数据  提交订单
...
CRONJOBS = [
    ...
    # 参数2:任务(函数)
    # 参数3: 日志路径(必须正确)
    ('*/60 * * * *', 'contents.crons.generate_static_index_html', '>> /home/python/meiduo/meiguo_34/mall/logs/crontab.log')
]
...
CACHES = {
    "default": {
        ...
        "LOCATION": "redis://127.0.0.1:6379/0",    # 代表预留库
        "OPTIONS": {
    ...
    "session": {
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://127.0.0.1:6379/1",  #  使用一号库  一共有16个库
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
    ...
    "history": {
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://127.0.0.1:6379/3",    # 3  代表几号库
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
...
SESSION_ENGINE = "django.contrib.sessions.backends.cache"  # 用cache 具体看课件
SESSION_CACHE_ALIAS = "session"   # session
...
JWT_AUTH = {
    ...
    'JWT_EXPIRATION_DELTA': datetime.timedelta(days=1),  #  指明token的有效期为1天
}
...
# 设置客户端访问storage
# FastDFS
FDFS_URL = 'http://192.168.217.178:8888/'  # 访问图片的路径域名 ip地址修改为自己机器的ip地址
...
HAYSTACK_CONNECTIONS = {
    'default': {
        ...
        'URL': 'http://192.168.217.178:9200/',  # 此处为elasticsearch运行的服务器ip地址，端口号固定为9200
